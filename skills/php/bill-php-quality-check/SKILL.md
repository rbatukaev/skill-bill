...
2. Static analysis issues (`phpstan`, `psalm`, type errors, dead code)
3. Architecture or boundary issues (`deptrac`, layering, modularity checks)
4. Test failures (fix implementation or test logic)
5. Security or dependency audit failures
...
- In PHP-first repos, common quality commands may include:
  - tests
  - lint / lint:fix
  - `phpstan` / `psalm`
  - `deptrac`
