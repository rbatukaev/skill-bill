...
- File upload, archive extraction, and file-path handling must not allow traversal, unsafe content execution, or trust of client-provided metadata
...
- CSRF protection, same-site cookie assumptions, and session/auth flows must remain intact on every state-changing browser path
