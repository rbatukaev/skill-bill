...
If `.agents/skill-overrides.md` exists in the project root and contains a `## bill-php-feature-implement` section, read that section and apply it as the highest-priority instruction for this skill. The matching section may refine or replace parts of the default workflow below.
...
Design Doc + Issue Key → Assess + Extract Criteria → [Size gate] → Create Branch →
  ...
  SMALL (≤5 tasks, ≤3 modules):
    Save Spec → [Read History] → Plan → Implement → bill-php-code-review (narrow scope, dynamic 2-7 agents) → Completeness Audit → Final Validation Gate (auto-selected) → [Write History if impactful] → PR Description

  MEDIUM (6-15 tasks, ≤6 modules):
    Save Spec → Read History → Plan → Implement →
    Compact → bill-php-code-review (dynamic 2-7 agents) → Completeness Audit → Final Validation Gate (auto-selected) → Write History → PR Description

  LARGE (>15 tasks or >6 modules):
    Save Spec → Read History → Feature Flag? → Plan (phased) → Implement →
    Compact → bill-php-code-review (dynamic 2-7 agents) → Compact → Completeness Audit → Final Validation Gate (auto-selected) → Write History → PR Description
```
...
4. **Determine feature size** — SMALL / MEDIUM / LARGE based on:
   ...
   - Whether sync/async/migration/replay is involved
...
- Preserve code blocks (data models, API contracts, SQL, schemas) verbatim
...
- Do not auto-apply a fixed prefix when proposing new flag names; only use a prefix if the user explicitly asks for it

### Discover Codebase Patterns

Explore the codebase concurrently with planning:
1. Read `CLAUDE.md`, `AGENTS.md`, and the matching `bill-php-feature-implement` section in `.agents/skill-overrides.md` when present — treat all standards as mandatory
...
5. Identify validation surfaces so the final gate is chosen automatically:
   - PHP project or modules → `bill-php-quality-check`
...
- Order tasks by dependency (data layer → domain → transport/presentation)
- Each task must reference which acceptance criteria it satisfies
- **The final task in every plan MUST be a dedicated test task.** This task writes tests covering the new/changed logic. It is never optional — if there is code to implement, there are tests to write. The `Tests:` field on this task must list the specific scenarios to cover (e.g., happy path, edge cases, guard clauses). Implementation tasks may set `Tests: None` only because testing is deferred to this final task.
...
### Final Validation
- Strategy: `bill-php-quality-check` | inline agent-config validation | both
...
## Step 4: Execute Plan
 ...
 - After each task, print progress: `✅ [3/10] Implemented PostRepository with transaction-safe writes`
...
- **When removing components, templates, routes, or code:** immediately clean up orphaned resources, bindings, imports, and other dead code in the same task
...
Run the `bill-php-code-review` skill and its matching `.agents/skill-overrides.md` section, then apply them inline. Treat that skill as the source of truth for:
...
- If Blocker or Major issues are found, auto-fix them and re-run `bill-php-code-review` (or only the affected specialists if that skill supports it)
- If only Minor issues remain, report them and continue
- When `bill-php-code-review` is invoked from this skill, do not stop to ask the user which finding to fix first
...
✅ #1: <criterion text>
   Implemented in: FileA.php:42, FileB.php:88
...
- **PHP/backend repos or modules touched:** run `bill-php-quality-check`
...
Do **not** create a separate utility skill just for this validation path unless the repository itself is explicitly centered on validation as a product concept. `bill-php-feature-implement` owns this behavior.
...
- `bill-php-code-review` — automatic after implementation; it inspects changed areas and selects 2-7 specialists dynamically
- `bill-php-quality-check` — when the affected repo/modules use the PHP validation path
- `bill-module-history` — writes/maintains module history for impactful or larger features
...
| Sync/migration/replay involved | No | No | Yes |
...
| Review agents | Dynamic (2-7, based on diff) | Dynamic (2-7, based on diff) | Dynamic (2-7, based on diff) |
